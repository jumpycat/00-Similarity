...
class ResNet(nn.Module):
    def __init__(self, block, layers):
        ...
        self.calsim_up = nn.Conv2d(512, 1, kernel_size=(2,1), stride=1, bias=True)
        self.calsim_left = nn.Conv2d(512, 1, kernel_size=(1,2), stride=1, bias=True)

        self.calsim_up_bank = nn.Conv2d(512, 1, kernel_size=(2, 1), stride=1, dilation=2,padding=1,bias=True)
        self.calsim_left_bank = nn.Conv2d(512, 1, kernel_size=(1, 2), stride=1, dilation=2,padding=1,bias=True)
...
class DealDataset(Dataset):
    def __init__(self, loader=default_loader):
        ...
        fake_root = r'/data2/Jianwei-Fei/00-Dataset/01-Images/00-FF++/Deepfakes/raw/train/'
...
# Deepfakes Face2Face FaceSwap NeuralTextures
fake_root = r'/data2/Jianwei-Fei/00-Dataset/01-Images/00-FF++/Deepfakes/raw/val/'
...
if __name__ == '__main__':
    for epoch in range(EPOCH):
        ...
        for i, data in enumerate(train_loader, 0):
            ...
            print(data)
            with open('logs/logs-raw-df-4.txt', 'a', encoding='utf-8') as f:
                f.write(data)
                f.write('\n')
        ...
        tag = 'raw-df-4-epoch-%03d-loss-%.03f-ValAcc-%.03f-Threshold-%.03f' % (epoch + 1,loss.item(),best_acc,best_th)
        print(tag)
        with open('logs-raw-df-4.txt', 'a', encoding='utf-8') as f:
            f.write(tag)
            f.write('\n')
