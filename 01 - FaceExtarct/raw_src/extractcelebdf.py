...
with open(path+'List_of_testing_videos.txt','r') as f:
    data=f.read()

splitdata=data.split('\n')
testdata=[a.split(' ')[1] for a in splitdata]
testlabel=[1-int(a.split(' ')[0]) for a in splitdata]
...
for video in data:
    ...
    target_path=path+'testimg/'+video
    ...
    if len(os.listdir(target_path))>=32:
        continue
    try:
        extract_frames(videopath,target_path)
    except:
        print('error',target_path)
    i+=1
    print(i,'/',datalen,video_name)
